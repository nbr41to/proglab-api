...
export const compileMonthlySummary = (params: {
  date: string;
  usersCounts: { userId: string; count: number }[];
  reactionsCounts: { reaction: string; count: number }[];
}) => {
  ...
  usersCounts.forEach((user) => {
    ...
    results.push(
      `${prefixEmoji} <@${user.userId}>さんは${user.count}回のリアクションをしました！\n`,
    );
  });
  results.push('=== === === ===\n');
  ...
  reactionsCounts.forEach((reaction) => {
    ...
    const prefixEmoji = rank === -1 ? '🎉' : prefixEmojis[rank];
    results.push(
      `${prefixEmoji} ${reaction.count}回「${reaction.reaction}」が使用されました！\n`,
    );
  });
  return results.join('\n');
};
...
const checkRank = (
  params: { count: number; [anyKey: string]: string | number }[],
) => {
